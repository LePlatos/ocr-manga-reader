...
// TODO: this is an ugly copy-paste of SDBrowserActivity
/** File picker for EPWING CATALOG/CATALOGS files */
public class SDEpwingBrowerActivity extends TabActivity
{
  ...
  protected void onCreate(Bundle savedInstanceState)
  {
    ...
    if (Environment.MEDIA_MOUNTED.equals(storageState))
    {
      ...
      if ((comicsPath != null) && (comicsPath.length() > 0))
      { // TODO simplify
        comicsPath = comicsPath.replaceFirst("CATALOGS$", "");
        comicsPath = comicsPath.replaceFirst("CATALOG$", "");
  ...
  private void changeDirectory(File directory)
  {
    ...
    browserListView.setAdapter(new ListAdapter(SDEpwingBrowerActivity.this, directory, R.layout.sd_item_empty));
  }
  ...
  protected Dialog onCreateDialog(int id)
  {
    ...
    {
      case NO_SD:
        return new AlertDialog.Builder(this).setIcon(android.R.drawable.ic_menu_info_details)
            .setTitle(R.string.dialog_no_sd_title).setMessage(R.string.dialog_no_sd_text)
            .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener()
            {
              public void onClick(DialogInterface dialog, int whichButton)
              {
                SDEpwingBrowerActivity.this.setResult(RESULT_CANCELED);
                SDEpwingBrowerActivity.this.finish();
              }
            }).create();
    }
    return null;
  ...
  public class ListAdapter extends BaseAdapter
  {
    ...
    private void filterContents()
    {
      ...
      if (allContents != null)
      {
        ...
        for (int i = 0; i < allContents.length; i++)
        {
          ...
          {
            if (contentName.equals("CATALOGS") || contentName.equals("CATALOG"))
            {
    ...
    public View getView(int position, View convertView, ViewGroup parent)
    {
      ...
      else
      {
        ...
        if (name.equals("CATALOGS") || name.equals("CATALOG"))
        {
          // iconId = supportedExtensions.get(extension);
          iconId = R.drawable.file_generic;
        }
        else
        {
  ...
  // HistoryManager.getInstance(SDEpwingBrowerActivity.this).getRecentFiles();
